(function (global, factory) {
	...
	function renderComponent(component) {
	  if (component.__P && component.__d) {
	    ...
	    diff(component.__P, newVNode, oldVNode, component.__n, component.__P.namespaceURI, newVNode._slotIndex, oldVNode.__u & MODE_HYDRATE ? [oldDom] : NULL, commitQueue, oldDom == NULL ? getDomSibling(oldVNode) : oldDom, !!(oldVNode.__u & MODE_HYDRATE), refQueue);
	  ...
	  if ((vnode = vnode.__) != NULL && vnode.__c != NULL) {
	    ...
	    vnode.__k.some(function (child) {
	      if (child != NULL && child.__e != NULL) {
	        return vnode.__e = vnode.__c.base = child.__e;
	      }
	    });
	...
	function diffChildren(parentDom, renderResult, newParentVNode, oldParentVNode, globalContext, namespace, slotIndex, excessDomChildren, commitQueue, oldDom, isHydrating, refQueue) {
	  ...
	  for (i = 0; i < newChildrenLength; i++) {
	    ...
	    var result = diff(parentDom, childVNode, oldVNode, globalContext, namespace,
	...
	function findMatchingIndex(childVNode, oldChildren, skewedIndex, remainingOldChildren) {
	  ...
	  if (oldVNode === NULL && key == null || matched && key == oldVNode.key && type == oldVNode.type && oldVNode._slotIndex === newSlot) {
	    return skewedIndex;
	    ...
	    while (x >= 0 || y < oldChildren.length) {
	      ...
	      if (oldVNode != NULL && (oldVNode.__u & MATCHED) == 0 && key == oldVNode.key && type == oldVNode.type && oldVNode._slotIndex === newSlot) {
	        return childIndex;
	      }
	    }
	  }
	...
	function diff(parentDom, newVNode, oldVNode, globalContext, namespace, slotIndex, excessDomChildren, commitQueue, oldDom, isHydrating, refQueue) {
	  ...
	  outer: if (typeof newType == 'function') {
	    try {
	      ...
	      oldDom = diffChildren(parentDom, isArray(renderResult) ? renderResult : [renderResult], newVNode, oldVNode, globalContext, namespace, slotIndex, excessDomChildren, commitQueue, oldDom, isHydrating, refQueue);
	      c.base = newVNode.__e;
	      ...
	      if (isHydrating || excessDomChildren != NULL) {
	        if (e.then) {
	          ...
	          for (var i = excessDomChildren.length; i--;) {
	...
	function cloneNode(node) {
	  if (typeof node != 'object' || node == NULL || node.__b > 0) {
	    return node;
	  }
	  if (isArray(node)) {
	...
	function diffElementNodes(dom, newVNode, oldVNode, globalContext, namespace, slotIndex, excessDomChildren, commitQueue, isHydrating, refQueue) {
	  ...
	  if (nodeType == NULL) {
	    ...
	    if (newHtml) {
	      ...
	      diffChildren(
	      // @ts-expect-error
	      newVNode.type == 'template' ? dom.content : dom, isArray(newChildren) ? newChildren : [newChildren], newVNode, oldVNode, globalContext, nodeType == 'foreignObject' ? XHTML_NAMESPACE : namespace, _slotIndex, excessDomChildren, commitQueue, excessDomChildren ? excessDomChildren[0] : oldVNode.__k && getDomSibling(oldVNode, 0), isHydrating, refQueue);
	    ...
	    if (!isHydrating) {
	      ...
	      nodeType == 'option' && inputValue != oldProps[i])) {
	        setProperty(dom, i, inputValue);
	      }
	      i = 'checked';
	      if (checked != UNDEFINED && checked != dom[i]) {
	        setProperty(dom, i, checked);
	      }
	    }
	  }
	  return dom;
	...
	function render(vnode, parentDom, replaceNode) {
	  ...
	  vnode, oldVNode || EMPTY_OBJ, EMPTY_OBJ, parentDom.namespaceURI, 0, !isHydrating && replaceNode ? [replaceNode] : oldVNode ? NULL : parentDom.firstChild ? slice.call(parentDom.childNodes) : NULL, commitQueue, !isHydrating && replaceNode ? replaceNode : oldVNode ? oldVNode.__e : parentDom.firstChild, isHydrating, refQueue);
