...
class ReaddirpStream extends Readable {
    constructor(options = {}){
        ...
        this._root = external_path_resolve(root);
    ...
    async _formatEntry(dirent, path) {
        ...
        try {
            const fullPath = external_path_resolve(join(path, basename));
            entry = {
                path: external_path_relative(this._root, fullPath),
                fullPath,
                basename
            };
...
const isIBMi = 'OS400' === external_os_type();
const EVENTS = {
    ALL: 'all',
    READY: 'ready',
    ADD: 'add',
...
class FSWatcher extends EventEmitter {
    ...
    _awaitWriteFinish(path, threshold, event, awfEmit) {
        ...
        function awaitWriteFinishFn(prevStat) {
            external_fs_stat(fullPath, (err, curStat)=>{
