...
class LynxTemplatePluginImpl {
    ...
    async #generateAsyncTemplate(compilation) {
        ...
        await Promise.all(Object.entries(asyncChunkGroups).map(([_entryName, chunkGroups]) => {
            const entryNames = // We use the chunk name(provided by `webpackChunkName`) as filename
             chunkGroups
                .filter(cg => cg.name !== null && cg.name !== undefined).map(cg => cg.name);
            const chunkNames = entryNames.map(name => hooks.asyncChunkName.call(name));
            ...
            const asyncAssetsInfoByGroups = this.#getAssetsInformationByFilenames(compilation, chunkGroups.flatMap(cg => cg.getFiles()).filter(chunkFile => predicateNonHotModuleReplacementAsset(chunkFile, compilation)));
            return this.#encodeByAssetsInformation(compilation, asyncAssetsInfoByGroups, entryNames, filenameTemplate, path.join(intermediateRoot, 'async', filename), 
            /** isAsync */ true);
        }));
    }
    async #encodeByAssetsInformation(compilation, assetsInfoByGroups, entryNames, filenameTemplate, intermediate, isAsync) {
        ...
        if (typeof compiler.options.output.publicPath === 'string'
            ...
            templateDebugUrl = new URL(debugInfoPath, compiler.options.output.publicPath).toString();
        }
        const encodeRawData = {
            compilerOptions: {
        ...
        if (isDebug() || isDev) {
            compilation.emitAsset(path.posix.format({
                dir: intermediatePosix,
                base: 'tasm.json',
    ...
    #getAssetsInformationByGroups(compilation, entryNames) {
        const filenames = entryNames.flatMap(entryName => {
            ...
            const entryPointUnfilteredFiles = compilation.entrypoints.get(entryName)
                .getFiles();
            return entryPointUnfilteredFiles.filter((chunkFile) => predicateNonHotModuleReplacementAsset(chunkFile, compilation));
        });
...
export function predicateNonHotModuleReplacementAsset(chunkFile, compilation) {
