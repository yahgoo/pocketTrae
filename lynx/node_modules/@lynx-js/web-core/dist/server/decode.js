...
export function decodeTemplate(buffer, transformVW, transformVH, transformREM) {
    if (buffer.length < 8) {
    ...
    while (offset < buffer.length) {
        ...
        switch (label) {
            ...
            case TemplateSectionLabel.StyleInfo: {
                const buffer = decode_style_info(content, config['isLazy'] === 'true' ? '' : undefined, // URL is not available in synchronous decode usually, or passed as arg? The user req says "uint8array as params decode directly". Assuming URL is empty or unneeded for sync server decode unless specified.
                config['enableCSSSelector'] === 'true', transformVW, transformVH, transformREM);
                styleInfo = buffer;
                break;
            }
